...
public class StageSetttingController
{
    ...
    public List<EnemyStageSpawnData> GetEnemySpawnData(int stageIndex, NodeType type)
    {
        return enemySpawnData
            .Where(x => x.stageIndex == stageIndex && x.type == type)
            .ToList();
    }
    ...
    public RandomEventData GetRandomEvent(int stageIndex)
    {
        var available = allRandomEvents
            .Where(x => x.stage == stageIndex && !usedRandomEvnent.Contains(x.index))
            .ToList();
