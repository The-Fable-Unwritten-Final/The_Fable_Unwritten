...
public class EnemySkillDatabase : MonoSingleton<EnemySkillDatabase>
{
    ...
    private Dictionary<int, EnemyAct> enemySkillDict = new();

    private void Start()
    {
        InitializeEnemySkillDictionary();
    }

    /// <summary>
    /// skillList를 기반으로 Dictionary 초기화
    /// </summary>
    private void InitializeEnemySkillDictionary()
    {
        enemySkillDict.Clear();

        foreach (var act in skillList)
        {
            if (act == null) continue;

            if (enemySkillDict.ContainsKey(act.index))
            {
                Debug.LogWarning($"[EnemySkillDatabase] 중복된 스킬 인덱스 감지: {act.index}");
                continue;
            }

            enemySkillDict[act.index] = act;
        }

        Debug.Log($"[EnemySkillDatabase] 총 {enemySkillDict.Count}개의 스킬 데이터 등록됨");
    }
    ...
    {
        if (enemySkillDict.TryGetValue(index, out var act))
        {
            return act;
        }
